...
class WeatherRouter extends Component {
  ...
  fetchWithId = (id) => {
    let lang = this.context;
    fetch(
      `https://api.openweathermap.org/data/2.5/weather?id=${id}&units=metric&APPID=${WEATHER_API}&lang=${lang}`
    )
      .then((results) => {
        return results.json();
      })
    ...
    const list = searchHistory.map((history, key) => (
      <>
        <span
          ...
          key={key}
          onClick={() => this.fetchWithId(history.id)}
        >
          {history.name}
        </span>
        <span
          className="historyRemove"
          onClick={() => this.handleRemove(history.id)}
        >
          x
        </span>
      </>
