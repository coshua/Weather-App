...
const measureUVI = (uvi, lang = "kr") => {
    ...
    if (lang === "kr") {
        ...
        uvi > 11 ? message = "Very high ultraviolet. Should avoid any kind of outdoor activity." :
            uvi > 8 ? message = "Strong ultraviolet. It is recommended to stay home. " :
                uvi > 6 ? message = "UV index is high. Wear long sleeves to protect your skin." :
                    message = "";
    }
    return message;
}
...
const measurePrecip = (daily, lang = "kr") => {
    ...
    if (daily.rain !== undefined)
        message += lang === "kr" ? `비가 올 예정입니다. 강수량은 시간당 ${daily.rain}mm 입니다.`
            : `It will be raining. Precipitation would be ${daily.rain}mm per hours.`
    if (daily.snow !== undefined)
        message += lang === "kr" ? `눈이 올 예정입니다. 강설량은 시간당 ${daily.snow}mm 입니다.`
...
const measureWindspeed = (wind, lang = "kr") => {
    ...
    if (lang === "kr") {
        ...
        wind > 13.9 ? message = "Strong winds. You may have difficulty walking straightforward." :
            wind > 10.8 ? message = "Blowing wind would able to shake trees." :
                wind > 5.5 ? message = "Gentle winds." :
...
const feelsLike = (daily, lang = "kr") => {
    var message = lang === "kr" ? `낮과 밤 동안의 체감 온도는 각각 ${Math.round(daily.feels_like.day * 10) / 10}°C, 
                ${Math.round(daily.feels_like.night * 10) / 10}°C 입니다. 일교차는 ${Math.round((daily.temp.max - daily.temp.min) * 10) / 10}
                °C 입니다.`
                : `Perceived temperatures during day and night are ${Math.round(daily.feels_like.day * 10) / 10}°C, 
                ${Math.round(daily.feels_like.night * 10) / 10}°C each. Daily cross would be ${Math.round((daily.temp.max - daily.temp.min) * 10) / 10}.`;
    return message;
}
...
const Forecast = ({ match, history, daily, city }) => {
    ...
    return (
        <div>
            ...
            {measurePrecip(specific).length > 0 ? (<>{measurePrecip(specific, lang)} <br /></>) : ('')}
            {measureUVI(specific.uvi, lang)} <br />
            {measureWindspeed(specific.wind_speed, lang)} <br />
            <div className="buttonGroup">
                {specific.dt !== daily[0].dt ?
                    (<NavLink to={`/weather/${calculateDay(-1)}`} > <button id="previous">{lang==="kr" ? "어제" : "Previous"}</button></NavLink>)
                    : ('')}
                {/* <button onClick={() => history.goBack()}>Home</button> */}
                <NavLink to={'/weather'}> <button className="center">{lang==="kr" ? "홈" : "Home"}</button> </NavLink>
                {specific.dt !== daily[7].dt ?
                    (<NavLink to={`/weather/${calculateDay(1)}`} > <button id="next">{lang==="kr" ? "내일" : "Next"}</button> </NavLink>)
                    : ('')}
            </div>
        </div>
    );
